...
pub fn constraint_solver(brc_flip: u32, shmid: i32, id: usize, lastone: u32) {
  ...
  info!("calling run_solver: shmid={}, pipeid={}, brc_flip={}, lastone={}", shmid, id, brc_flip, lastone);
  let pipeid_u32: u32 = id as u32;
  unsafe { run_solver(shmid, pipeid_u32, brc_flip, lastone) };
}
...
pub fn ce_loop_sync(
  ...
  while running.load(Ordering::Relaxed) {
    ...
    while let Some((buf, path, queue_id, seed_id, lastone)) = depot.get_next_input_rare() {
      ...
      // Always trigger solver first so it blocks waiting for scheduler decisions
          constraint_solver(brc_strategy, shmid, executor_id, lastone);

      // Then run tracking to feed coverage/pc tokens
    ...
    // If no inputs were present, still prompt a CE decision round to keep solving continuously
    if depot.qlen() == 0 {
      constraint_solver(brc_strategy, shmid, executor_id, 1u32);
    }
