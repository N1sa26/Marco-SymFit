    ...
    while let Some(item) = q.pop_max() {
        ...
        if b {            
          // info!("queueid: {}", item.queue_id);
            // Force CE solving each iteration: always mark lastone=1 to prompt decisions
            return Some((read_from_file(&item.path), item.path, item.queue_id, item.seed_id, 1u32))
        }
    }
