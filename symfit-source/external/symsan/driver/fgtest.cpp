...
static void generate_input(z3::model &m) {
  // Marco-SymFit: Disable symsan's own solver output generation
  // FastGen handles test case generation, so we should not generate files here
  // Check if SYMCC_OUTPUT_DIR is set (Marco-SymFit mode) to disable file generation
  const char* symcc_output = getenv("SYMCC_OUTPUT_DIR");
  const char* marco_mode = getenv("MARCO_MODE");
  if ((symcc_output && symcc_output[0] != '\0') || (marco_mode && strcmp(marco_mode, "1") == 0)) {
    // Marco-SymFit mode: skip file generation, FastGen will handle it
    AOUT("Marco-SymFit: skipping symsan solver output generation (FastGen handles it)\n");
    return;
  }
